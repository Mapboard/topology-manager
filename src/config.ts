...
if (connection == null) { connection = { host, port, database }; } // Also needs user, password
if (data_schema == null) { data_schema = 'map_digitizer'; }
if (topo_schema == null) { topo_schema = 'map_topology'; }
...
const config = {connection, data_schema, configDir,
...
config.extensions = (() => {
  ...
  for (let k in extensions) {
    ...
    cfg.path = loc;
    if (cfg.commands == null) { cfg.commands = []; }
    result.push(cfg);
  }
  return result;
})();
