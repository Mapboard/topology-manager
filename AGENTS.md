...
3. `update_faces` — resolves dirty faces into `map_face` polygons. Python pops one dirty face at a time and calls the PL/pgSQL `dissolve_component(seed, map_layer)`, which lazily expands that face's joinable component; the dirty faces it swallowed are dropped from the remaining list so each component is dissolved once. `--incremental` (default on) checkpoints: results are persisted and committed every `persist_interval` iterations (default 100) instead of all at the end. The components are identical either way; only the commit cadence differs.
...
- Face dissolving (`fixtures/07-get-adjacent-faces.sql`). The joinable face graph comes from `joinable_face_edges(map_layer)`: an edge is crossable when `layers_are_joinable(...) OR faces_are_joinable(...)` — linework relies on the first term (a contact line blocks the join), map-area mode on the second (faces sharing a resolved identity join even across another map's footprint edge). The default `faces_are_joinable` (the `search` strategy, `fixtures/identity/search.sql`) returns **false** so linework reduces to `layers_are_joinable` alone; the `direct` strategy compares stored identities. Components are **not** computed once per layer. `dissolve_component(_seed, _map_layer, _barrier_layers)` runs a per-seed BFS: it holds the component and frontier in indexed session temp tables and, each iteration, pulls only the `edge_data` rows incident to the current frontier, so cost scales with the component rather than the layer. It returns the component's faces plus the map_faces they replace, so only O(component) rows cross the wire. Python (`commands/update_faces/__init__.py`, with `_get_adjacent_faces_core` in `helpers.py`) drives the loop: pop a dirty face, dissolve its component, filter the dissolved faces out of the remaining dirty list, repeat. `joinable_face_edges(map_layer)` materializes the whole layer's joinable graph and is available for whole-layer analysis, but the update loop does not use it. `adjacent_faces(face_id, _map_layer)` is a thin SQL wrapper returning just the faces of one component. Checkpointing (`--incremental`) is safe because persisting a map_face does **not** change the dissolve graph — it only adds `relation` rows over existing primitives; joinability comes from `__edge_relation` + boundary identity, never from `map_face` contents. (A future strategy that fed persisted face identity back into `faces_are_joinable` would break that invariant — that's the deferred "reactive graph" case.)
