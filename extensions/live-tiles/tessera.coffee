    ...
    tilelive.load sourceURI, (err, source) ->
      ...
      getInfo source, (err, info) ->
        ...
        source.getTile z, x, y, (err, data, headers) ->
          ...
          if ext == 'pbf'
            headers['content-type'] = headers['content-type'] or 'application/x-protobuf'
            headers['content-encoding'] = headers['content-encoding'] or 'gzip'
  ...
  app.get '/index.json', (req, res, next) ->
    ...
    tilelive.load uri, (err, source) ->
      ...
      getInfo source, (err, info) ->
        ...
        uri = protocol + '://' + host + (path.dirname(req.originalUrl) + tilePath.replace('{format}', getExtension(info.format))).replace(/\/+/g, '/')
        info.tiles = [ uri ]
        info.tilejson = '2.0.0'
