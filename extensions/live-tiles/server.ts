...
const {db, sql} = require('../../src/util.coffee');
...
const liveTileServer = function(cfg){
  ...
  app.get("/style.json", async function(req, res){
    const fn = require.resolve("../server/map-digitizer-server/sql/get-feature-types.sql");
    const polygonTypes = await db.query(sql(fn), {schema: 'map_digitizer', table: 'polygon_type'});
